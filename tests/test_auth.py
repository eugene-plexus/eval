"""Tests for v0.2 bearer auth on the eval component.
...
from eugene_plexus_eval.auth_state import AuthState
...
@pytest.fixture
def coordinator_service_token(signing_key: bytes) -> str:
    return _issue(signing_key=signing_key, sub="coordinator", aud="service:coordinator")
...
def test_missing_bearer_rejects_with_401(authed_client: TestClient) -> None:
    ...
    assert response.json()["detail"]["component"] == "eval"
...
# --------------------------------------------------------------------------- #
# Operator audience accepted on operator routes + eval-control routes
# --------------------------------------------------------------------------- #
...
def test_operator_token_accepted_on_suites(authed_client: TestClient, operator_token: str) -> None:
    response = authed_client.get(
        "/v1/eval/suites", headers={"Authorization": f"Bearer {operator_token}"}
    )
    assert response.status_code == 200


# --------------------------------------------------------------------------- #
# Service audience rejected on config, accepted on eval control
# --------------------------------------------------------------------------- #


def test_service_token_rejected_on_config_patch(
    authed_client: TestClient, coordinator_service_token: str
) -> None:
    """A compromised coordinator must not be able to rewrite the eval
    component's config (e.g. repoint the eval output root)."""
    response = authed_client.patch(
        "/v1/config",
        json={"minLogitEntropy": 0.1},
        headers={"Authorization": f"Bearer {coordinator_service_token}"},
    )
    assert response.status_code == 401


def test_service_token_accepted_on_suites(
    authed_client: TestClient, coordinator_service_token: str
) -> None:
    """Normal production path: the coordinator lists/controls eval suites
    using its service:coordinator token."""
    response = authed_client.get(
        "/v1/eval/suites",
        headers={"Authorization": f"Bearer {coordinator_service_token}"},
    )
...
def test_load_auth_state_disabled_when_no_signing_key() -> None:
    from eugene_plexus_eval.auth_state import load_auth_state
