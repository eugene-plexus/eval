...
`src/eugene_plexus_eval/_generated/`.
...
GENERATED_DIR = REPO_ROOT / "src" / "eugene_plexus_eval" / "_generated"
WORKING_DIR = REPO_ROOT / ".codegen-cache"

SPECS_TARBALL_URL_TEMPLATE = "https://github.com/eugene-plexus/specs/archive/{ref}.tar.gz"

# eval.yaml hosts the eval component's path operations and inlines several
# of its request/response bodies under `paths` (the suites-list response,
# the run/compare request bodies, the compare response), while the named
# schemas it $refs — EvalSuite, EvalResult — and all the shared platform
# schemas live in components/common.yaml, a complete OpenAPI document
# (with `paths: {}`). We generate from both: common.yaml ->
# common_models.py for the shared/eval schemas, and eval.yaml -> models.py
# for the path-local inline shapes. The resulting modules include a
# handful of types the eval component doesn't strictly need — that's fine,
# they're cheap and keep the generator's job simple.
SPECS_TO_GENERATE = [
    ("openapi/eval.yaml", "models.py"),
...
def run_codegen(specs_root: Path, ref: str) -> None:
    ...
    for input_rel, output_name in SPECS_TO_GENERATE:
        ...
        cmd = [
            ...
            "--disable-timestamp",
            # eval.yaml defines its request/response shapes inline under
            # `paths` (its named schemas live in components alongside the
            # shared platform schemas in common.yaml). Without `paths` scope
            # the generator finds nothing in eval.yaml and errors out.
            # Mirrors connector's codegen, which feeds a paths-bearing spec
            # the same way.
            "--openapi-scopes",
            "paths",
            "schemas",
