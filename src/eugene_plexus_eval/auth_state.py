"""Auth state for the eval component's verify-only role.

Built once at startup from the env vars the watchdog threads in when
it spawns this child:

  * `EUGENE_PLEXUS_EVAL_AUTH_SIGNING_KEY` — base64 of the 32-byte HMAC
    key used to validate inbound bearer tokens.
  * `EUGENE_PLEXUS_EVAL_SERVICE_TOKEN` — long-lived JWT (`aud:
    service:eval`). The watchdog supplies this for symmetry with
    other kinds; a future engine can present it for outbound peer
    calls (e.g. reporting eval-run completion to the coordinator).
  * `EUGENE_PLEXUS_EVAL_MASTER_KEY` — base64 of the 32-byte secretbox
    key. Not used in the v0.3 skeleton — no eval secrets are
...
def load_auth_state(
    *,
    ...
    if signing_key is None:
        ...
        log.warning(
            "EUGENE_PLEXUS_EVAL_AUTH_SIGNING_KEY not set — running unauthenticated "
