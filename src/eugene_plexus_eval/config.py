...
# Schema for the eval component's config surface. The v0.3 skeleton carries
# the knobs the eval engine will consult once it lands: where eval runs
# write their working output, the default sampling temperature for the
# sample-review generations, and the sampled-token-entropy floor below
# which a checkpoint is flagged as mode-collapsed. Ports are owned by the
# watchdog topology now, passed to spawned children via
# EUGENE_PLEXUS_EVAL_BIND_PORT — never a config field.
FIELDS: list[ConfigField] = [
    ConfigField(
        key="evalRoot",
        label="Eval output root",
        description=(
            "Filesystem directory on the eval host where eval runs write "
            "their working output (logged sample generations, per-run "
            "metric records, comparison reports). Created automatically if "
            "absent. Use an absolute path; relative paths resolve against "
            "the eval component's working directory."
        ),
        category="paths",
    ...
    ConfigField(
        ...
        description=(
            "Default temperature used when generating sample-review "
            "completions for a checkpoint. Lower values make the sampled "
            "text more deterministic; higher values increase diversity. An "
            "eval suite or run request may override this per run."
        ),
        category="sampling",
    ...
    ConfigField(
        key="minLogitEntropy",
        label="Minimum sampled-token entropy (nats)",
        description=(
            "Lower bound on mean sampled-token entropy (in nats) below "
            "which a checkpoint is flagged as mode-collapsed during a "
            "token-entropy eval. Checkpoints whose sampled distribution "
            "collapses below this floor are reported but not necessarily "
            "failed — the threshold is advisory. Tune to match the model "
            "family's healthy entropy band."
        ),
        category="collapse",
        valueType=ConfigValueType.number,
        default=1.0,
    ...
    ConfigField(
        ...
        label="Log level",
        description=(
            "How chatty the eval service's terminal output is. "
            "`DEBUG` prints detailed eval-run-lifecycle and per-prompt "
            "events; `INFO` is the normal level; `WARNING` and `ERROR` go "
