...
For the eval component: eval control (`/v1/eval/*`) accepts operator OR
service tokens — the coordinator presents `service:coordinator` to run
suites / compare checkpoints, and operators drive evals from the UI.
Config edits are operator-only.
"""
...
def require_authorized(
    ...
    /v1/eval/* control, which the coordinator (service) drives and the
    operator may drive through the UI."""
