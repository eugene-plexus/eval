...
class EvalSuite(BaseModel):
    """
    v0.3-draft. A named set of eval prompts + metrics to run against a
    checkpoint. Referenced from `common.yaml#/components/schemas/EvalSuiteRef`.
    ...
    validationDatasetId: UUID | None = Field(
        None, description='Optional dataset for perplexity / val-loss evaluation.'
    )
...
class EvalResult(BaseModel):
    """
    v0.3-draft. Outcome of running a suite against one checkpoint.
    """
    ...
    tokenEntropy: float | None = None
    samples: list[Sample] | None = None


class V1EvalSuitesGetResponse(BaseModel):
    suites: list[EvalSuite]


class V1EvalRunsPostRequest(BaseModel):
    evalSuiteId: UUID
    checkpointId: UUID
...
class V1EvalComparePostRequest(BaseModel):
    evalSuiteId: UUID
    baselineCheckpointId: UUID
    candidateCheckpointId: UUID


class V1EvalComparePostResponse(BaseModel):
    baseline: EvalResult | None = None
    candidate: EvalResult | None = None


class ProblemModel(RootModel[Any]):
    root: Any
