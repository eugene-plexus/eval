# Contributing to Eugene Plexus `eval`

Thanks for your interest. This service implements the `eval` OpenAPI contract from [`eugene-plexus/specs`](https://github.com/eugene-plexus/specs) — please read this before opening a PR.
...
- **Engine work** — the eval-execution engine, suite running, perplexity / val-loss, token-entropy collapse detection, checkpoint comparison.
...
```bash
git clone https://github.com/eugene-plexus/eval
cd eval
python -m venv .venv
...
File issues at <https://github.com/eugene-plexus/eval/issues>. Useful issues include:

- Concrete eval-run failures with reproduction steps.
- Spec-vs-impl divergence (the impl drifted from `eval.yaml`).
